    ...
    def train(self, dataloader, learning_rate, total_epochs):
        ...
        for epoch in pbar:
            ...
            for idx, data in enumerate(dataloader):
                ...
                z, reconstructed_x, balance_loss, cv_distinctive_loss = self.model(feature_list)
                reconstruction_loss=0
                for i in range(self.num_view):
                    reconstruction_loss += loss_fn(data[i],reconstructed_x[i])


                loss = reconstruction_loss+self.lamda*balance_loss + self.gamma* cv_distinctive_loss
                # Total loss
                loss_all += loss.item()
                loss.backward()
                self.optimizer.step()
